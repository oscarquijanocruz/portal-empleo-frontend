...
export default function SupportSection() {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-0 to-blue-100 rounded-lg shadow-md p-6">
      {/* Barra de búsqueda */}
      <div className="flex flex-col items-center mb-8">
        ...
        <div className="flex items-center space-x-4 w-full md:w-1/2 bg-white rounded-full shadow-md p-2">
          <input
            ...
            style={{
              background: "linear-gradient(to right, #ffffffff, #f0f9ffff)", 
              borderRadius: "50px", 
            }}
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          <button className="bg-blue-300 text-white px-4 py-2 rounded-full hover:bg-blue-500">
            <Search size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {categories.map((category) => (
          <div key={category.name} className="bg-white p-6 rounded-xl shadow-md hover:bg-green-50 transition-all">
      ...
      <div className="mt-12 text-center">
        <button className="bg-green-400 text-white px-6 py-3 rounded-lg text-lg flex items-center justify-center mx-auto hover:bg-purple-500 transition-all">
